...
impl Ctx2 {
    ...
    pub async fn new() -> Self {
        ...
        let (bucket, cleanup) = ctx.create_bucket_helper().await;
    ...
    pub async fn new_with_object(object_lock: bool) -> Self {
        ...
        let _resp: CreateBucketResponse = ctx
            ...
            .await
            .unwrap();
        let cleanup = CleanupGuard::new(ctx.client.clone(), bucket.clone());
        ...
        let _resp: PutObjectContentResponse = ctx
            .client
            .put_object_content(&bucket, &object, data)
            .unwrap()
            .build()
            .send()
            .await
    ...
    pub async fn new_aux(&mut self) -> BucketName {
        let bucket = rand_bucket_name();
        self.aux_bucket = Some(bucket.clone());
        self._aux_cleanup = Some(CleanupGuard::new(self.client.clone(), bucket.clone()));
