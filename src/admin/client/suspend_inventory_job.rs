...
/// Argument builder for suspending an inventory job.
///
/// This pauses a job and prevents it from being scheduled in the future.
/// The job can be resumed later with [`MinioAdminClient::resume_inventory_job`].
#[derive(Clone, Debug, TypedBuilder)]
pub struct SuspendInventoryJob {
    #[builder(!default)]
...
/// Builder type for [`SuspendInventoryJob`].
pub type SuspendInventoryJobBldr =
    SuspendInventoryJobBuilder<((MinioAdminClient,), (String,), (String,))>;

impl AdminApi for SuspendInventoryJob {
    type Response = AdminInventoryControlResponse;
}

impl ToAdminRequest for SuspendInventoryJob {
    fn to_admin_request(self) -> Result<S3Request, ValidationErr> {
        ...
        let path = format!(
            "/minio/admin/v3/inventory/{}/{}/suspend",
            self.bucket, self.id
        );
...
impl MinioAdminClient {
    ...
    ///
    ///     let resp = admin
    ///         .suspend_inventory_job("my-bucket", "daily-job")
    ///         .build().send().await.unwrap();
    ///     println!("Status: {}", resp.status());
    /// }
    /// ```
    pub fn suspend_inventory_job<S1: Into<String>, S2: Into<String>>(
        &self,
        bucket: S1,
        id: S2,
    ) -> SuspendInventoryJobBldr {
        SuspendInventoryJob::builder()
