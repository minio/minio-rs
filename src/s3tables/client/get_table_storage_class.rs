...
//! Client method for GetTableStorageClass operation

use crate::s3::error::ValidationErr;
use crate::s3tables::builders::{GetTableStorageClass, GetTableStorageClassBldr};
use crate::s3tables::client::TablesClient;
use crate::s3tables::utils::{Namespace, TableName, WarehouseName};

impl TablesClient {
    /// Gets the storage class for a table
    ///
    /// # Arguments
    ///
    ...
    ///
    /// let response = client
    ///     .get_table_storage_class(&warehouse_name, &namespace, &table_name)?
    ///     .build()
    ///     .send()
    ///     .await?;
    ///
    ...
    /// # }
    /// ```
    pub fn get_table_storage_class<W, N, T>(
        &self,
        warehouse: W,
        namespace: N,
        table: T,
    ) -> Result<GetTableStorageClassBldr, ValidationErr>
    where
    ...
    {
        Ok(GetTableStorageClass::builder()
            .client(self.clone())
