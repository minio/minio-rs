...
//! Client method for PutTableReplication operation

use crate::s3::error::ValidationErr;
use crate::s3tables::builders::{PutTableReplication, PutTableReplicationBldr};
use crate::s3tables::client::TablesClient;
use crate::s3tables::types::ReplicationConfiguration;
use crate::s3tables::utils::{Namespace, TableName, WarehouseName};

impl TablesClient {
    /// Sets the replication configuration for a table
    ///
    /// # Arguments
    ///
    ...
    ///
    /// client
    ///     .put_table_replication(&warehouse_name, &namespace, &table_name, config)?
    ///     .build()
    ///     .send()
    ///     .await?;
    /// # Ok(())
    /// # }
    /// ```
    pub fn put_table_replication<W, N, T>(
        &self,
        warehouse: W,
        namespace: N,
        table: T,
        replication_configuration: ReplicationConfiguration,
    ) -> Result<PutTableReplicationBldr, ValidationErr>
    where
    ...
    {
        Ok(PutTableReplication::builder()
            .client(self.clone())
