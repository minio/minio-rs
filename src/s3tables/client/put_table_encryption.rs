...
//! Client method for PutTableEncryption operation

use crate::s3::error::ValidationErr;
use crate::s3tables::builders::{PutTableEncryption, PutTableEncryptionBldr};
...
impl TablesClient {
    ...
    ///
    /// client
    ///     .put_table_encryption(&warehouse_name, &namespace, &table_name, encryption)?
    ///     .build()
    ///     .send()
    ///     .await?;
    /// # Ok(())
    /// # }
    /// ```
    pub fn put_table_encryption<W, N, T>(
        &self,
        warehouse: W,
        namespace: N,
        table: T,
        encryption_configuration: EncryptionConfiguration,
    ) -> Result<PutTableEncryptionBldr, ValidationErr>
    where
