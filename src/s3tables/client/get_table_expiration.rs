...
//! Client method for GetTableExpiration operation

use crate::s3::error::ValidationErr;
use crate::s3tables::builders::{GetTableExpiration, GetTableExpirationBldr};
use crate::s3tables::client::TablesClient;
use crate::s3tables::utils::{Namespace, TableName, WarehouseName};

impl TablesClient {
    /// Gets the record expiration configuration for a table
    ///
    /// # Arguments
    ///
    ...
    /// use minio::s3tables::response_traits::HasExpirationConfiguration;
    ///
    ...
    ///
    /// let response = client
    ///     .get_table_expiration(&warehouse_name, &namespace, &table_name)?
    ///     .build()
    ///     .send()
    ///     .await?;
    ///
    /// let config = response.expiration_configuration()?;
    /// println!("Expiration enabled: {}", config.is_enabled());
    /// # Ok(())
    /// # }
    /// ```
    pub fn get_table_expiration<W, N, T>(
        &self,
        warehouse: W,
        namespace: N,
        table: T,
    ) -> Result<GetTableExpirationBldr, ValidationErr>
    where
