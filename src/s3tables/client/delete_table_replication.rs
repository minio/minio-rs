...
//! Client method for DeleteTableReplication operation

use crate::s3::error::ValidationErr;
use crate::s3tables::builders::{DeleteTableReplication, DeleteTableReplicationBldr};
...
impl TablesClient {
    ...
    ///
    /// client
    ///     .delete_table_replication(&warehouse_name, &namespace, &table_name)?
    ///     .build()
    ///     .send()
    ///     .await?;
    /// # Ok(())
    /// # }
    /// ```
    pub fn delete_table_replication<W, N, T>(
        &self,
        warehouse: W,
        namespace: N,
        table: T,
    ) -> Result<DeleteTableReplicationBldr, ValidationErr>
    where
    ...
    {
        Ok(DeleteTableReplication::builder()
            .client(self.clone())
