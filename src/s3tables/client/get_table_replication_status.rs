...
//! Client method for GetTableReplicationStatus operation

use crate::s3::error::ValidationErr;
use crate::s3tables::builders::{GetTableReplicationStatus, GetTableReplicationStatusBldr};
use crate::s3tables::client::TablesClient;
use crate::s3tables::utils::{Namespace, TableName, WarehouseName};

impl TablesClient {
    /// Gets the replication status for a table
    ///
    /// # Arguments
    ///
    ...
    /// use minio::s3tables::response_traits::HasReplicationStatus;
    ///
    ...
    ///
    /// let response = client
    ///     .get_table_replication_status(&warehouse_name, &namespace, &table_name)?
    ///     .build()
    ///     .send()
    ///     .await?;
    ///
    /// let status = response.replication_status()?;
    /// println!("Status: {:?}", status);
    /// # Ok(())
    /// # }
    /// ```
    pub fn get_table_replication_status<W, N, T>(
        &self,
        warehouse: W,
        namespace: N,
        table: T,
    ) -> Result<GetTableReplicationStatusBldr, ValidationErr>
    where
    ...
    {
        Ok(GetTableReplicationStatus::builder()
            .client(self.clone())
