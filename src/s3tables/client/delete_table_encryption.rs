...
//! Client method for DeleteTableEncryption operation

use crate::s3::error::ValidationErr;
use crate::s3tables::builders::{DeleteTableEncryption, DeleteTableEncryptionBldr};
...
impl TablesClient {
    ...
    ///
    /// This reverts the table to using the default encryption settings
    ...
    ///
    /// client
    ///     .delete_table_encryption(&warehouse_name, &namespace, &table_name)?
    ///     .build()
    ///     .send()
    ///     .await?;
    /// # Ok(())
    /// # }
    /// ```
    pub fn delete_table_encryption<W, N, T>(
        &self,
        warehouse: W,
        namespace: N,
        table: T,
    ) -> Result<DeleteTableEncryptionBldr, ValidationErr>
    where
    ...
    {
        Ok(DeleteTableEncryption::builder()
            .client(self.clone())
