...
impl TablesClient {
    ...
    /// # Arguments
    ///
    /// * `warehouse` - Warehouse name (or string to validate)
    ///
    /// # Optional Parameters
    ///
    ...
    ///     .await?;
    ///
    /// println!("Created warehouse: {}", response.warehouse()?);
    /// # Ok(())
    /// # }
    /// ```
    pub fn create_warehouse<W>(&self, warehouse: W) -> Result<CreateWarehouseBldr, ValidationErr>
    where
        W: TryInto<WarehouseName>,
        W::Error: Into<ValidationErr>,
    {
        Ok(CreateWarehouse::builder()
            .client(self.clone())
