...
//! Client method for GetTableReplication operation

use crate::s3::error::ValidationErr;
use crate::s3tables::builders::{GetTableReplication, GetTableReplicationBldr};
use crate::s3tables::client::TablesClient;
use crate::s3tables::utils::{Namespace, TableName, WarehouseName};

impl TablesClient {
    /// Gets the replication configuration for a table
    ///
    /// # Arguments
    ///
    ...
    ///
    /// let response = client
    ///     .get_table_replication(&warehouse_name, &namespace, &table_name)?
    ///     .build()
    ///     .send()
    ///     .await?;
    ///
    ...
    /// # }
    /// ```
    pub fn get_table_replication<W, N, T>(
        &self,
        warehouse: W,
        namespace: N,
        table: T,
    ) -> Result<GetTableReplicationBldr, ValidationErr>
    where
    ...
    {
        Ok(GetTableReplication::builder()
            .client(self.clone())
