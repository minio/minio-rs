...
//! Client method for GetTableExpirationJobStatus operation

use crate::s3::error::ValidationErr;
use crate::s3tables::builders::{GetTableExpirationJobStatus, GetTableExpirationJobStatusBldr};
use crate::s3tables::client::TablesClient;
use crate::s3tables::utils::{Namespace, TableName, WarehouseName};

impl TablesClient {
    /// Gets the record expiration job status for a table
    ///
    /// # Arguments
    ///
    ...
    /// use minio::s3tables::response_traits::HasExpirationJobStatus;
    ///
    ...
    ///
    /// let response = client
    ///     .get_table_expiration_job_status(&warehouse_name, &namespace, &table_name)?
    ///     .build()
    ///     .send()
    ///     .await?;
    ///
    /// let status = response.expiration_job_status()?;
    /// println!("Job status: {:?}", status);
    /// # Ok(())
    /// # }
    /// ```
    pub fn get_table_expiration_job_status<W, N, T>(
        &self,
        warehouse: W,
        namespace: N,
        table: T,
    ) -> Result<GetTableExpirationJobStatusBldr, ValidationErr>
    where
    ...
    {
        Ok(GetTableExpirationJobStatus::builder()
            .client(self.clone())
