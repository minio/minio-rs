...
impl TablesClient {
    ...
    /// # Arguments
    ///
    /// * `warehouse` - Name of the warehouse to retrieve (or string to validate)
    ///
    /// # Example
    ///
    /// ```no_run
    ...
    ///     .await?;
    ///
    /// println!("Warehouse: {}", response.warehouse()?);
    /// println!("Bucket: {}", response.bucket()?);
    /// println!("Created: {}", response.created_at()?);
    /// # Ok(())
    /// # }
    /// ```
    pub fn get_warehouse<W>(&self, warehouse: W) -> Result<GetWarehouseBldr, ValidationErr>
    where
        W: TryInto<WarehouseName>,
        W::Error: Into<ValidationErr>,
    {
        Ok(GetWarehouse::builder()
            .client(self.clone())
