...
//! Response type for AdvCommitTable operation
...
/// Response from AdvCommitTable operation
///
/// Follows the lazy evaluation pattern: stores raw response data and parses fields on demand.
#[derive(Clone, Debug)]
pub struct AdvCommitTableResponse {
    request: TablesRequest,
    headers: HeaderMap,
    body: Bytes,
}

impl AdvCommitTableResponse {}

impl_has_tables_fields!(AdvCommitTableResponse);
impl_from_tables_response!(AdvCommitTableResponse);

impl HasTableResult for AdvCommitTableResponse {}

impl HasTableMetadata for AdvCommitTableResponse {
    ...
    fn metadata_location(&self) -> Result<MetadataLocation, ValidationErr> {
        self.table_result()?
            ...
            .ok_or_else(|| ValidationErr::StrError {
                message: "Missing 'metadata-location' field in AdvCommitTable response".into(),
                source: None,
            })
    }
}
