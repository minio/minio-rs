...
//! Authentication providers for Iceberg REST Catalog backends
//!
//! This module provides pluggable authentication for different Iceberg catalog
//! implementations. The SDK supports:
//!
//! - **AWS SigV4** (default): For MinIO AIStor, AWS S3 Tables, and S3-compatible backends
//! - **Bearer Token**: For OAuth2-based catalogs like Apache Polaris, Nessie, Gravitino
//!
//! # Choosing an Authentication Method
//!
//! | Backend | Recommended Auth | Notes |
//! |---------|-----------------|-------|
//! | MinIO AIStor | `SigV4Auth` | Default, uses S3 credentials |
//! | AWS S3 Tables | `SigV4Auth` | Default, uses AWS credentials |
//! | Apache Polaris | `BearerAuth` | OAuth2 token from identity provider |
//! | Project Nessie | `BearerAuth` | OAuth2/OIDC token |
//! | Gravitino | `BearerAuth` | OAuth2 token |
//! | Tabular | `BearerAuth` | OAuth2 token |
//!
//! # Example: Using Bearer Authentication with Polaris
//!
//! ```no_run
//! use minio::s3tables::auth::BearerAuth;
//! use minio::s3tables::GenericTablesClient;
//!
//! # async fn example() -> Result<(), Box<dyn std::error::Error>> {
//! // Obtain token from your OAuth2 provider
//! let token = "your-oauth2-access-token";
//!
//! let client = GenericTablesClient::builder()
//!     .base_url("https://polaris.example.com")
//!     .base_path("/api/catalog/v1")
//!     .auth(BearerAuth::new(token))
//!     .build()?;
//!
//! // Use the client for catalog operations
//! # Ok(())
//! # }
//! ```
...
/// Bearer token authentication for OAuth2-based catalogs
///
/// This authentication method is used with Iceberg REST Catalog implementations
/// that support OAuth2/OIDC authentication, including:
///
/// - Apache Polaris
/// - Project Nessie
/// - Gravitino
