...
impl ListWarehousesResponse {
    ...
    pub fn warehouses(&self) -> Result<Vec<WarehouseName>, ValidationErr> {
        #[derive(serde::Deserialize)]
        struct WarehousesWrapper {
            warehouses: Vec<String>,
        }

        let wrapper = serde_json::from_slice::<WarehousesWrapper>(&self.body)
            .map_err(ValidationErr::JsonError)?;

        Ok(wrapper
            .warehouses
            .into_iter()
            .map(WarehouseName::new_unchecked)
            .collect())
    }
}
