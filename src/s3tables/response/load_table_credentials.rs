...
impl StorageCredential {
    ...
    /// Returns true if the credentials will expire within the given duration
...
mod tests {
    ...
    fn test_storage_credential_parsing() {
        ...
        assert!(cred.expiration_time.is_some());
        let exp = cred.expiration_time.unwrap();
        assert_eq!(exp.year(), 2025);
    ...
    fn test_expires_within() {
        ...
        let cred = StorageCredential {
            ...
            session_token: None,
            expiration_time: Some(future),
        };

        // Expires within 2 hours
        assert!(cred.expires_within(chrono::Duration::hours(2)));
        // Does not expire within 30 minutes
