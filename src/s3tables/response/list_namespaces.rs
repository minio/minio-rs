...
/// # Available Fields
///
/// - [`namespaces()`](Self::namespaces) - Returns the list of namespaces
...
impl ListNamespacesResponse {
    ...
    pub fn namespaces(&self) -> Result<Vec<Namespace>, ValidationErr> {
        #[derive(serde::Deserialize)]
        struct NamespacesWrapper {
            namespaces: Vec<Vec<String>>,
        }

        let raw_namespaces = match serde_json::from_slice::<NamespacesWrapper>(&self.body) {
            Ok(wrapper) => wrapper.namespaces,
            Err(_) => {
