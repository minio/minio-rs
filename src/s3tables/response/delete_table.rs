...
//! # Specification
//!
//! Implements the response for `DELETE /v1/{prefix}/namespaces/{namespace}/tables/{table}` from the
//! [Apache Iceberg REST Catalog API](https://github.com/apache/iceberg/blob/main/open-api/rest-catalog-open-api.yaml).
//!
//! ## Response (HTTP 204)
//!
//! Returns no content on successful deletion. The `purgeRequested` query parameter
//! controls whether the underlying data files are also deleted.
//!
//! ## Response Schema
//!
...
/// # Specification
///
/// Implements `DELETE /v1/{prefix}/namespaces/{namespace}/tables/{table}` (HTTP 204 response) from the
/// [Apache Iceberg REST Catalog API](https://github.com/apache/iceberg/blob/main/open-api/rest-catalog-open-api.yaml).
///
/// # Note
///
...
impl_has_tables_fields!(DeleteTableResponse);
impl_from_tables_response_cached!(DeleteTableResponse);
impl_has_cached_body!(DeleteTableResponse);

impl HasWarehouseName for DeleteTableResponse {}
