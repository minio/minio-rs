...
pub enum PrimitiveType {
    ...
    /// Timestamp without timezone with nanosecond precision (V3)
    ///
    /// Enhanced timestamp type that stores time with nanosecond precision
    /// instead of the microsecond precision of `Timestamp`.
    #[serde(rename = "timestamp_ns")]
    TimestampNs,

    /// Timestamp with timezone with nanosecond precision (V3)
    ///
    /// Enhanced timestamp type that stores time with nanosecond precision
    /// instead of the microsecond precision of `Timestamptz`.
...
///
/// Serializes as Iceberg transform strings (e.g., "identity", "bucket[16]", "truncate[10]")
...
impl std::str::FromStr for Transform {
    ...
    /// Parse a transform from its string representation
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        ...
        if s == "identity" {
            ...
            Ok(Transform::Void)
        } else if s.starts_with("bucket[") && s.ends_with(']') {
            let n_str = &s[7..s.len() - 1];
            let n = n_str
                .parse::<u32>()
                .map_err(|_| format!("Invalid bucket count: {}", n_str))?;
            Ok(Transform::Bucket { n })
        } else if s.starts_with("truncate[") && s.ends_with(']') {
            let w_str = &s[9..s.len() - 1];
            let width = w_str
                .parse::<u32>()
                .map_err(|_| format!("Invalid truncate width: {}", w_str))?;
            Ok(Transform::Truncate { width })
        } else {
            Err(format!("Unknown transform: {}", s))
        }
    }
}
...
pub struct V3Features {
    ...
    /// Table has nanosecond precision timestamp columns
...
impl V3Features {
    ...
    pub fn feature_list(&self) -> Vec<&'static str> {
        ...
        if self.nanosecond_timestamps {
            features.push("nanosecond timestamps");
        }
        features
    }
}
        ...
        match field_type {
            FieldType::Primitive(p) => match p {
                ...
                PrimitiveType::TimestampNs | PrimitiveType::TimestamptzNs => {
                    features.nanosecond_timestamps = true
                }
                _ => {}
            },
            FieldType::Struct(s) => {
...
mod tests {
    ...
    #[test]
    fn test_primitive_type_timestamp_ns() {
        let pt = PrimitiveType::TimestampNs;
        let json = serde_json::to_string(&pt).unwrap();
        assert_eq!(json, "\"timestamp_ns\"");

        let deserialized: PrimitiveType = serde_json::from_str("\"timestamp_ns\"").unwrap();
        assert!(matches!(deserialized, PrimitiveType::TimestampNs));
    }

    #[test]
    fn test_primitive_type_timestamptz_ns() {
        let pt = PrimitiveType::TimestamptzNs;
        let json = serde_json::to_string(&pt).unwrap();
        assert_eq!(json, "\"timestamptz_ns\"");

        let deserialized: PrimitiveType = serde_json::from_str("\"timestamptz_ns\"").unwrap();
        assert!(matches!(deserialized, PrimitiveType::TimestamptzNs));
    }

    #[test]
    fn test_schema_has_v3_types_nanosecond_timestamps() {
        let schema = Schema {
            schema_id: Some(1),
            fields: vec![
                Field {
                    ...
                    required: true,
                    field_type: FieldType::Primitive(PrimitiveType::TimestampNs),
                    doc: None,
                    initial_default: None,
                    write_default: None,
                },
                Field {
                    ...
                    required: false,
                    field_type: FieldType::Primitive(PrimitiveType::TimestamptzNs),
                    doc: None,
                    initial_default: None,
                    write_default: None,
                },
        ...
        let features = format_version_utils::schema_has_v3_types(&schema);
        assert!(features.nanosecond_timestamps);
        assert!(!features.variant_types);
        assert!(!features.geometry_types);
        assert_eq!(features.required_format_version(), format_version::V3);
    }
