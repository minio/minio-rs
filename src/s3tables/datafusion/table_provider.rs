...
impl MinioTableProvider {
    ...
    pub fn new(
        ...
        // Register ObjectStore for s3:// URLs to enable ListingTable
        let s3_url = url::Url::parse("s3://").expect("s3:// is a valid URL");
        session_state
            .runtime_env()
...
impl TableProvider for MinioTableProvider {
    ...
    /// * `_limit` - Optional row limit (unused in current implementation)
    ///
    /// # Returns
    ...
    async fn scan(
        ...
        let warehouse = WarehouseName::new(self.warehouse_name.clone()).map_err(|e| {
        ...
        let plan_resp = if let Some(filter) = filter_json {
            // Server-side filtering: apply Iceberg filter expression
        ...
        let execution_plans = self
            .build_execution_plans(&file_scan_tasks, projection, &schema, &residual_filters)
            .map_err(|e| {
                DataFusionError::External(Box::new(std::io::Error::other(format!(
...
mod tests {
    ...
    fn test_translate_single_comparison_filter() {
        ...
        // Verify the filter JSON was generated correctly
    ...
    fn test_classify_scalar_function_residual() {
        ...
        let (pushable, residual) = MinioTableProvider::classify_filters(std::slice::from_ref(&expr));
        assert_eq!(pushable.len(), 1);
        assert_eq!(residual.len(), 0);
    }
