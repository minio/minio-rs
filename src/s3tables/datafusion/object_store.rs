...
use crate::s3::types::{PartInfo, S3Api, ToStream};
...
impl MultipartUpload for MinioMultipartUpload {
    ...
    async fn abort(&mut self) -> Result<()> {
        self.client
            .abort_multipart_upload(&self.bucket, &self.object, &self.upload_id)
            .build()
            .send()
            .await
            .map_err(Self::convert_error)?;
...
impl MinioObjectStore {
    ...
    pub fn new(client: Arc<MinioClient>, bucket: String) -> Self {
        Self { client, bucket }
    }

    /// Helper to convert object_store::Path to MinIO object name.
    fn path_to_key(&self, location: &Path) -> String {
        location.to_string()
    }
...
impl ObjectStore for MinioObjectStore {
    async fn put(&self, location: &Path, payload: PutPayload) -> Result<PutResult> {
        ...
        let response: PutObjectResponse = self
            .client
            .put_object(&self.bucket, &key, SegmentedBytes::from(Bytes::from(data)))
            .build()
            .send()
            .await
            .map_err(Self::convert_error)?;
    ...
    async fn put_opts(
        ...
        let key: String = self.path_to_key(location);
...
mod tests {
    ...
    fn test_path_conversion() {
        ...
        assert_eq!(store.path_to_key(&path), "data/test.parquet");
    }
    ...
    fn test_path_conversion_root() {
        ...
        assert_eq!(store.path_to_key(&path), "test.parquet");
    }
    ...
    fn test_path_conversion_nested() {
        ...
        assert_eq!(
            store.path_to_key(&path),
            "data/year=2024/month=12/test.parquet"
        );
    }
    ...
    fn test_creation() {
        ...
        let store = MinioObjectStore::new(client.clone(), "my-bucket".to_string());
    ...
    fn test_multipart_upload_structure() {
        ...
        let upload = MinioMultipartUpload::new(
            client,
            "test-bucket".to_string(),
            "test-object".to_string(),
            "upload-id-123".to_string(),
        );

        assert_eq!(upload.bucket, "test-bucket");
        assert_eq!(upload.object, "test-object");
        assert_eq!(upload.upload_id, "upload-id-123");
