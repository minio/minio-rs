...
use crate::s3::types::header_constants::CONTENT_TYPE;
use crate::s3::types::{BucketName, Region, S3Api, S3Request, ToS3Request};
use crate::s3::utils::insert;
use crate::s3inventory::{InventoryJobId, JobDefinition, PutInventoryConfigResponse};
use bytes::Bytes;
use http::Method;
use std::sync::Arc;
...
impl ToS3Request for PutInventoryConfig {
    fn to_s3request(self) -> Result<S3Request, ValidationErr> {
        ...
        if self.job_definition.id != self.id.as_str() {
        ...
        let yaml_body = crate::s3inventory::serialize_job_definition(&self.job_definition)
            .map_err(|e| match e {
        ...
        headers.insert(CONTENT_TYPE.to_string(), "application/yaml".to_string());
        ...
        Ok(S3Request::builder()
            ...
            .headers(headers)
            .body(Arc::new(SegmentedBytes::from(Bytes::from(yaml_body))))
            .build())
    }
}
