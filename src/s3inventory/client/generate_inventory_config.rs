...
use crate::s3inventory::builders::{GenerateInventoryConfig, GenerateInventoryConfigBldr};
...
impl MinioClient {
    ...
    /// which returns a [`Result`] containing a [`GenerateInventoryConfigResponse`](crate::s3inventory::GenerateInventoryConfigResponse).
    ///
    /// # Arguments
    ///
    ...
    ///
    ///     let resp = client
    ///         .generate_inventory_config("my-bucket", "daily-job").unwrap()
    ///         .build().send().await.unwrap();
    ///     println!("Template: {}", resp.yaml_template().unwrap());
    /// }
    /// ```
    pub fn generate_inventory_config<B, I>(
        &self,
        bucket: B,
        id: I,
    ) -> Result<GenerateInventoryConfigBldr, ValidationErr>
    where
    ...
    {
        Ok(GenerateInventoryConfig::builder()
            .client(self.clone())
