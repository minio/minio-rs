...
impl MinioClient {
    ...
    ///         .get_object_lock_config("bucket-name").unwrap()
    ///         .build().send().await.unwrap();
    ///     println!("retrieved object lock config '{:?}' from bucket '{}' is enabled", resp.config(), resp.bucket().unwrap());
    /// }
    /// ```
    pub fn get_object_lock_config<B>(
        &self,
        bucket: B,
    ) -> Result<GetObjectLockConfigBldr, ValidationErr>
    where
        B: TryInto<BucketName>,
        B::Error: Into<ValidationErr>,
    {
        Ok(GetObjectLockConfig::builder()
            .client(self.clone())
