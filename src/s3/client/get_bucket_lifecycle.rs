...
impl MinioClient {
    ...
    ///         .get_bucket_lifecycle("bucket-name").unwrap()
    ///         .build().send().await.unwrap();
    ///     println!("retrieved bucket lifecycle config '{:?}' from bucket '{}'", resp.config(), resp.bucket().unwrap());
    /// }
    /// ```
    pub fn get_bucket_lifecycle<B>(
        &self,
        bucket: B,
    ) -> Result<GetBucketLifecycleBldr, ValidationErr>
    where
        B: TryInto<BucketName>,
        B::Error: Into<ValidationErr>,
    {
        Ok(GetBucketLifecycle::builder()
            .client(self.clone())
