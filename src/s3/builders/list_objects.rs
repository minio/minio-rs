...
use crate::s3::{
    ...
    types::{Execute, ExecuteStream, Request, ToRequest},
...
#[async_trait]
impl ExecuteStream for ListObjectsV1 {
...
impl ToRequest for ListObjectsV1 {
    fn to_request<'a>(&'a self) -> Result<Request<'a>, Error> {
...
impl Execute for ListObjectsV1 {
    type Response = ListObjectsV1Response;
}
...
#[async_trait]
impl ExecuteStream for ListObjectsV2 {
...
impl Execute for ListObjectsV2 {
    type Response = ListObjectsV2Response;
}

impl ToRequest for ListObjectsV2 {
    fn to_request(&self) -> Result<Request, Error> {
        check_bucket_name(&self.bucket, true)?;
...
#[async_trait]
impl ExecuteStream for ListObjectVersions {
...
impl Execute for ListObjectVersions {
    type Response = ListObjectVersionsResponse;
}

impl ToRequest for ListObjectVersions {
    fn to_request(&self) -> Result<Request, Error> {
        check_bucket_name(&self.bucket, true)?;
...
#[async_trait]
impl ExecuteStream for ListObjects {
    ...
    async fn execute_stream(
        ...
        if self.use_api_v1 {
            let stream = ListObjectsV1::from(self).execute_stream().await;
            Box::new(stream.map(|v| v.map(|v| v.into())))
        } else if self.include_versions {
            let stream = ListObjectVersions::from(self).execute_stream().await;
            Box::new(stream.map(|v| v.map(|v| v.into())))
        } else {
            let stream = ListObjectsV2::from(self).execute_stream().await;
            Box::new(stream.map(|v| v.map(|v| v.into())))
        }
    }
}
