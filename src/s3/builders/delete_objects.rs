...
impl<K: ValidKey> From<K> for ObjectToDelete {
    fn from(key: K) -> Self {
        ...
        Self {
            key: ObjectKey::new(key_str).expect("valid object key"),
            version_id: None,
        }
    }
}
...
impl<K: ValidKey> From<(K, &VersionId)> for ObjectToDelete {
    fn from((key, version_id): (K, &VersionId)) -> Self {
        let key_str: String = key.into();
        Self {
            key: ObjectKey::new(key_str).expect("valid object key"),
...
impl<K: ValidKey> From<(K, Option<&VersionId>)> for ObjectToDelete {
    fn from((key, version_id): (K, Option<&VersionId>)) -> Self {
        let key_str: String = key.into();
        Self {
            key: ObjectKey::new(key_str).expect("valid object key"),
...
impl From<ListEntry> for ObjectToDelete {
    fn from(entry: ListEntry) -> Self {
        Self {
            key: ObjectKey::new(entry.name).expect("valid object key"),
            version_id: entry
                .version_id
...
impl From<DeleteError> for ObjectToDelete {
    fn from(entry: DeleteError) -> Self {
        Self {
            key: ObjectKey::new(entry.object_name).expect("valid object key"),
            version_id: entry
                .version_id
                .map(|v| VersionId::new(v).expect("valid version id")),
        }
