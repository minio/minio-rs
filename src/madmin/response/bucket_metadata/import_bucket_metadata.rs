...
use crate::madmin::types::bucket_metadata::BucketMetaImportErrs;
use crate::s3::error::ValidationErr;
use bytes::Bytes;
use http::HeaderMap;

/// Response for the ImportBucketMetadata API operation.
///
/// Contains import status for various metadata categories.
#[derive(Debug, Clone)]
...
impl_from_madmin_response!(ImportBucketMetadataResponse);
impl_has_madmin_fields!(ImportBucketMetadataResponse);
impl HasBucket for ImportBucketMetadataResponse {}

impl ImportBucketMetadataResponse {
    /// Returns the import errors and status per bucket.
    pub fn result(&self) -> Result<BucketMetaImportErrs, ValidationErr> {
