...
/// Response containing profiling data as binary ZIP archive.
#[derive(Debug, Clone)]
pub struct ProfileResponse(pub Vec<u8>);

impl Deref for ProfileResponse {
    type Target = Vec<u8>;
...
#[async_trait]
impl FromMadminResponse for ProfileResponse {
    async fn from_madmin_response(
        ...
        Ok(ProfileResponse(data.to_vec()))
    }
}
