...
/// Response from the ListAccessKeysLDAPBulk admin API operation.
///
/// Maps LDAP user DNs to their access key lists.
#[derive(Debug, Clone)]
pub struct ListAccessKeysLDAPBulkResponse {
    pub users_keys: HashMap<String, ListAccessKeysResp>,
}

#[async_trait]
impl FromMadminResponse for ListAccessKeysLDAPBulkResponse {
    async fn from_madmin_response(
        ...
        let users_keys: HashMap<String, ListAccessKeysResp> =
            serde_json::from_slice(&decrypted_data)
                .map_err(|e| Error::Validation(ValidationErr::JsonError(e)))?;

        Ok(ListAccessKeysLDAPBulkResponse { users_keys })
    }
}
