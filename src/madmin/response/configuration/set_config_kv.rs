...
#[derive(Debug, Clone)]
pub struct SetConfigKVResponse {
    request: MadminRequest,
    headers: HeaderMap,
    body: Bytes,
}

impl_from_madmin_response!(SetConfigKVResponse);
impl_has_madmin_fields!(SetConfigKVResponse);

impl SetConfigKVResponse {
    /// Returns whether a server restart is required to apply the config changes.
    pub fn restart_required(&self) -> bool {
        self.headers
            .get(X_MINIO_CONFIG_APPLIED)
            .and_then(|v| v.to_str().ok())
            .map(|v| v != "true") //TODO would a case insensitive compare be better here?
            .unwrap_or(true)
    }
}
