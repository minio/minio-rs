...
#[derive(Debug, Clone)]
pub struct DetachPolicyResponse(pub PolicyAssociationResp);

impl Deref for DetachPolicyResponse {
...
#[async_trait]
impl FromMadminResponse for DetachPolicyResponse {
    async fn from_madmin_response(
        ...
        let password: String = request
            ...
            .ok_or_else(|| {
                Error::Validation(ValidationErr::StrError {
                    message: "Credentials required for DetachPolicy response".to_string(),
                    source: None,
                })
            })?
            .fetch()
        ...
        Ok(DetachPolicyResponse(response_data))
    }
}
