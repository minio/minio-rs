...
/// Response from the DetachPolicyLDAP admin API operation.
///
/// Contains information about policies that were successfully detached from LDAP users or groups.
#[derive(Debug, Clone)]
pub struct DetachPolicyLDAPResponse(pub PolicyAssociationResp);

impl DetachPolicyLDAPResponse {
...
impl Deref for DetachPolicyLDAPResponse {
...
#[async_trait]
impl FromMadminResponse for DetachPolicyLDAPResponse {
    async fn from_madmin_response(
        ...
        let password: String = request
            ...
            .ok_or_else(|| {
                Error::Validation(ValidationErr::StrError {
                    message: "Credentials required for DetachPolicyLDAP response".to_string(),
                    source: None,
                })
            })?
            .fetch()
        ...
        Ok(DetachPolicyLDAPResponse(response_data))
    }
}
