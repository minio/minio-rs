...
pub type ResumeInventoryJobBldr =
    ResumeInventoryJobBuilder<((MinioAdminClient,), (), (), (BucketName,), (InventoryJobId,))>;

impl MadminApi for ResumeInventoryJob {
    type MadminResponse = AdminInventoryControlResponse;
}

impl ToMadminRequest for ResumeInventoryJob {
    fn to_madmin_request(self) -> Result<MadminRequest, Error> {
        let path = format!(
            "/minio/admin/v4/inventory/{}/{}/resume",
            self.bucket.as_str(),
            self.id.as_str()
        );
