...
use crate::madmin::response::DetachPolicyResponse;
...
/// Builder type for [`DetachPolicy`].
pub type DetachPolicyBldr = DetachPolicyBuilder<((MadminClient,), (), (), ())>;

impl ToMadminRequest for DetachPolicy {
    fn to_madmin_request(self) -> Result<MadminRequest, Error> {
        ...
        let password = self
            ...
            .ok_or_else(|| {
                Error::Validation(ValidationErr::StrError {
                    message: "Credentials required for DetachPolicy".to_string(),
                    source: None,
                })
            })?
            .fetch()
        ...
        Ok(MadminRequest::builder()
            .client(self.client)
            .method(Method::POST)
            .path("/idp/builtin/policy/detach")
...
impl MadminApi for DetachPolicy {
    type MadminResponse = DetachPolicyResponse;
}
