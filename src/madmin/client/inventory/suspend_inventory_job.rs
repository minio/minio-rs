...
use crate::madmin::builders::inventory::{SuspendInventoryJob, SuspendInventoryJobBldr};
...
impl MinioAdminClient {
    ...
    ///
    ///     let resp = admin
    ///         .suspend_inventory_job(BucketName::new("my-bucket").unwrap(), "daily-job")
    ///         .build().send().await.unwrap();
    ///     println!("Status: {:?}", resp.admin_control());
    /// }
    /// ```
    pub fn suspend_inventory_job<S: Into<String>>(
        &self,
        bucket: BucketName,
        id: S,
