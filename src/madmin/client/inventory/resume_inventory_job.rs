...
use crate::madmin::builders::inventory::{ResumeInventoryJob, ResumeInventoryJobBldr};
...
impl MinioAdminClient {
    ...
    ///
    ///     let resp = admin
    ///         .resume_inventory_job(BucketName::new("my-bucket").unwrap(), "daily-job")
    ///         .build().send().await.unwrap();
    ///     println!("Status: {:?}", resp.admin_control());
    /// }
    /// ```
    pub fn resume_inventory_job<S: Into<String>>(
        &self,
        bucket: BucketName,
        id: S,
