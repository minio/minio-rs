...
use crate::madmin::builders::{AddUser, AddUserBldr};
use crate::madmin::madmin_client::MadminClient;
use crate::madmin::types::typed_parameters::{AccessKey, SecretKey};
use crate::s3::error::ValidationErr;

impl MadminClient {
    /// Add a new user to the MinIO server
    ///
    /// # Example
    ///
    /// ```no_run
    ...
    ///     .send()
    ///     .await?;
    /// println!("User added: {}", response.success);
    /// # Ok(())
    /// # }
    /// ```
    pub fn add_user<A, S>(&self, access_key: A, secret_key: S) -> Result<AddUserBldr, ValidationErr>
    where
        A: TryInto<AccessKey>,
