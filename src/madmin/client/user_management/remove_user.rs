...
use crate::madmin::builders::{RemoveUser, RemoveUserBldr};
use crate::madmin::madmin_client::MadminClient;
use crate::madmin::types::typed_parameters::AccessKey;
use crate::s3::error::ValidationErr;

impl MadminClient {
    /// Remove a user from the MinIO server
    ///
    /// # Example
    ///
    /// ```no_run
    ...
    ///     .send()
    ///     .await?;
    /// println!("User removed: {}", response.success);
    /// # Ok(())
    /// # }
    /// ```
    pub fn remove_user<A>(&self, access_key: A) -> Result<RemoveUserBldr, ValidationErr>
    where
        A: TryInto<AccessKey>,
        A::Error: Into<ValidationErr>,
    {
        Ok(RemoveUser::builder()
            .client(self.clone())
