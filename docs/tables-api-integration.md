...
### Server Limitations (Implemented but Limited)
...
| **Server Status** | Returns error: "multi-level namespaces are not supported" |

The SDK correctly implements multi-level namespace encoding per Iceberg spec, but MinIO AIStor only supports single-level namespaces (e.g., `["my_namespace"]`).
