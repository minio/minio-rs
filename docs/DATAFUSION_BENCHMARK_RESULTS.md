...
# Setup test data
cargo run --example datafusion_benchmark --release -- setup \
...
# Run benchmark
cargo run --example datafusion_benchmark --release -- bench \
  --endpoint http://localhost:9000 \
  --access-key minioadmin \
  --secret-key minioadmin \
  --bucket benchmark-test \
...
# Cleanup
cargo run --example datafusion_benchmark --release -- cleanup \
  --endpoint http://localhost:9000 \
  --access-key minioadmin \
  --secret-key minioadmin \
  --bucket benchmark-test
