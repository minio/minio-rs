...
async fn object_lock_config(ctx: TestContext, bucket: BucketName) {
    ...
    let resp: DeleteObjectLockConfigResponse = ctx
        .client
        .delete_object_lock_config(&bucket)
        .unwrap()
        .build()
        .send()
        .await
