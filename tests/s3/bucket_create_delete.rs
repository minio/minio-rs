...
async fn bucket_create(ctx: TestContext) -> Result<(), Error> {
    ...
    let resp: Result<CreateBucketResponse, Error> =
        ctx.client.create_bucket(&bucket)?.build().send().await;
    match resp {
...
#[minio_macros::test(no_bucket)]
async fn bucket_delete(ctx: TestContext) -> Result<(), Error> {
    let bucket = rand_bucket_name();

    // try to remove a bucket that does not exist
    let resp: Result<DeleteBucketResponse, Error> =
        ctx.client.delete_bucket(&bucket)?.build().send().await;
    match resp {
...
async fn test_bucket_delete_and_purge(
    ctx: &TestContext,
    bucket: &str,
    object: &str,
) -> Result<(), Error> {
    ...
    let resp: Result<DeleteBucketResponse, Error> = ctx
        .client
        .delete_bucket(BucketName::try_from(bucket).unwrap())
        .unwrap()
        .build()
        .send()
        .await;
    ...
    let resp: DeleteBucketResponse = ctx
        .client
        .delete_and_purge_bucket(BucketName::try_from(bucket).unwrap())
        .await
        .unwrap();
    assert_eq!(resp.bucket(), Some(&BucketName::try_from(bucket).unwrap()));
    Ok(())
...
#[minio_macros::test]
async fn bucket_delete_and_purge_1(ctx: TestContext, bucket: BucketName) -> Result<(), Error> {
    let object = rand_object_name_utf8(20);
    test_bucket_delete_and_purge(&ctx, bucket.as_str(), object.as_str()).await
}

/// Test purging a bucket with an object that contains white space characters.
#[minio_macros::test]
async fn bucket_delete_and_purge_2(ctx: TestContext, bucket: BucketName) -> Result<(), Error> {
    test_bucket_delete_and_purge(&ctx, bucket.as_str(), "a b+c").await
}
