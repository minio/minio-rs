...
async fn table_register(ctx: TestContext) {
    ...
    let metadata_location = table_result.metadata_location.clone().unwrap();
    assert!(
        metadata_location
            .as_str()
    ...
    match register_resp {
        Ok(register_resp) => {
            ...
            let load_resp: LoadTableResponse = tables
                .load_table(&warehouse, &namespace, registered_table_name.clone())
                .unwrap()
                .build()
                .send()
                .await
            ...
            tables
                .delete_table(&warehouse, &namespace, registered_table_name.clone())
                .unwrap()
                .build()
                .send()
                .await
