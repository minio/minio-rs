...
async fn metadata_location_set_on_create(ctx: TestContext) {
    ...
    let metadata_location = result.metadata_location.clone().unwrap();
    assert!(
        !metadata_location.as_str().is_empty(),
...
async fn metadata_location_format(ctx: TestContext) {
    ...
    let result = resp.table_result().unwrap();
    let metadata_location = result.metadata_location.clone().unwrap();
    let location_str = metadata_location.as_str();
    ...
    assert!(
        location_str.starts_with("s3://") || location_str.starts_with("s3a://"),
        "Metadata location should have S3 scheme, got: {}",
        location_str
    );
    assert!(
        location_str.contains("metadata"),
...
async fn metadata_location_unique_per_table(ctx: TestContext) {
    ...
    // Cleanup
    tables
        .delete_table(&warehouse, &namespace, table1.clone())
        .unwrap()
        .build()
        .send()
        .await
        .unwrap();
    tables
        .delete_table(&warehouse, &namespace, table2.clone())
        .unwrap()
        .build()
        .send()
        .await
