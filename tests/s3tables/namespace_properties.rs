...
use minio::s3tables::{
    HasNamespace, HasNamespacesResponse, HasProperties, TablesApi, TablesClient,
};
...
async fn namespace_properties(ctx: TestContext) {
    ...
    assert_eq!(create_resp.namespace(), namespace_name);

    // Verify response content
    assert_eq!(
        create_resp.namespaces_from_result().unwrap(),
    ...
    // Verify trait methods
    assert_eq!(get_resp.namespace(), namespace_name);

    // Verify response content
    assert_eq!(
        get_resp.namespaces_from_result().unwrap(),
        vec![namespace_name.clone()]
    );
