...
async fn table_load(ctx: TestContext) {
    ...
    assert_eq!(table_results.metadata_location.clone().unwrap(), location);

    // Test loading with snapshots parameter (SnapshotMode::Refs)
    let resp_refs: LoadTableResponse = tables
        .load_table(
            warehouse_name.clone(),
            namespace.clone(),
            table_name.clone(),
        ...
        .await
        .unwrap();
    let refs_results = resp_refs.table_result().unwrap();
    assert_eq!(
        refs_results.metadata_location.unwrap(),
        table_results.metadata_location.unwrap()
    );

    // Test loading with snapshots parameter (SnapshotMode::All)
    let resp_all: LoadTableResponse = tables
        .load_table(
            warehouse_name.clone(),
            namespace.clone(),
            table_name.clone(),
        ...
        .await
        .unwrap();
    let all_results = resp_all.table_result().unwrap();
    assert!(all_results.metadata_location.is_some());
