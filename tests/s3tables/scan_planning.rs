...
async fn plan_table_scan_basic(ctx: TestContext) {
    ...
    let resp: Result<PlanTableScanResponse, Error> = tables
        .plan_table_scan(&warehouse, &namespace, &table)
        .unwrap()
        .build()
        .send()
        .await;
        ...
        Ok(resp) => {
            ...
            if let Some(plan_id) = result.plan_id {
                let fetch_resp = tables
                    .fetch_planning_result(&warehouse, &namespace, &table, plan_id)
                    .unwrap()
                    .build()
                    .send()
                    .await
    ...
    match resp {
        Ok(resp) => {
            ...
            if result.status == PlanningStatus::Completed && !result.plan_tasks.is_empty() {
                ...
                let fetch_resp = tables
                    .fetch_scan_tasks(&warehouse, &namespace, &table, result.plan_tasks[0].clone())
                    .unwrap()
                    .build()
                    .send()
                    .await;
        ...
        Ok(resp) => {
            ...
            if let Some(plan_id) = result.plan_id {
                let cancel_resp = tables
                    .cancel_planning(&warehouse, &namespace, &table, plan_id)
                    .unwrap()
                    .build()
                    .send()
                    .await;
