...
async fn delete_and_purge_warehouse(ctx: TestContext) {
    ...
    let resp: DeleteWarehouseResponse = tables
        .delete_and_purge_warehouse(&warehouse)
        .await
    ...
    {
        Ok(_) => {
            panic!("Warehouse {warehouse} should have been deleted but still exists!");
        }
        Err(_) => {
...
async fn table_create_delete(ctx: TestContext) {
    ...
    let resp: CreateTableResponse = tables
        .create_table(&warehouse, &namespace, &table, schema.clone())
        .unwrap()
        .build()
        .send()
        .await
...
/// Test multi-level namespace support.
/// Note: MinIO may not fully support multi-level namespaces as AWS S3 Tables does.
/// This test verifies behavior and skips assertions if not supported.
#[minio_macros::test(no_bucket)]
async fn namespace_multi_level(ctx: TestContext) {
    ...
    let namespace = Namespace::try_from(vec![ns1.first().to_string(), ns2, ns3]).unwrap();
    ...
    let resp = match create_result {
        ...
        Err(e) => {
            eprintln!(
                "> Server failed to create multi-level namespace: {:?}. Skipping test.",
                e
            );
            delete_warehouse_helper(&warehouse, &tables).await;
            return;
    ...
    let returned_parts = resp.namespace_parts().unwrap();
    if returned_parts != namespace.as_slice() {
        // Server doesn't support full multi-level namespaces
        eprintln!(
            "> Server returns flattened namespace (got {:?}, expected {:?}). Skipping multi-level assertions.",
            returned_parts,
            namespace.as_slice()
        );
        // Clean up with what the server actually returned
        let actual_namespace = Namespace::try_from(returned_parts.to_vec()).unwrap();
        tables
            .delete_namespace(&warehouse, actual_namespace)
            .unwrap()
            .build()
            .send()
            .await
    ...
    assert_eq!(resp.identifiers().unwrap()[0].name, table.as_str());
    assert_eq!(
...
async fn list_operations(ctx: TestContext) {
    ...
    let resp: CreateTableResponse = tables
        .create_table(&warehouse, &namespace, &table2, schema.clone())
        .unwrap()
        .build()
        .send()
        .await
    ...
    let resp: ListNamespacesResponse = tables
        ...
        .await
        .unwrap();
    assert!(resp.namespaces().unwrap().iter().any(|ns| ns == &namespace));
    ...
    //assert!(warehouse_names.contains(&warehouse.as_str().to_string()));
