...
async fn namespace_create_delete(ctx: TestContext) {
    ...
    let resp: GetNamespaceResponse = tables
        ...
        .await
        .unwrap();
    assert_eq!(resp.namespaces_from_result().unwrap(), namespace.as_slice());
...
async fn list_operations(ctx: TestContext) {
    ...
    println!("warehouse_name = {}", warehouse_name.as_str());

    //TODO unknown why the warehouse is not in the list
    //assert!(warehouses_vec.contains(&warehouse_name.as_str().to_string()));
    ...
    if false {
        for v in warehouses_vec {
            if v.starts_with("warehouse") {
                ...
                let wh_name = WarehouseName::try_from(v.as_str()).unwrap();
                tables
                    .delete_and_purge_warehouse(wh_name)
                    .await
