...
//! Advanced (Tier 2) rename table tests using the builder pattern directly.
//!
//! These tests demonstrate the advanced API that provides more granular control
//! over rename operations compared to the simple TablesApi trait methods.
...
/// Test advanced rename table with namespace change using the Tier 2 builder API.
/// This demonstrates the capability to move tables between namespaces using
/// the more granular builder pattern instead of the simple TablesApi method.
...
async fn advanced_rename_table_with_namespace_change(ctx: TestContext) {
    ...
    let dest_namespace = rand_namespace();
    ...
    tables
        .create_namespace(&warehouse, dest_namespace.clone())
        .unwrap()
        .build()
        .send()
        .await
    ...
    let _rename_resp = RenameTable::builder()
        ...
        .dest_namespace(dest_namespace.clone())
        .dest_table(new_table_name.clone())
        .build()
        .send()
        .await
        .unwrap();
    ...
    let load_resp: LoadTableResponse = tables
        .load_table(&warehouse, dest_namespace.clone(), new_table_name.clone())
        .unwrap()
        .build()
        .send()
        .await
    ...
    assert_eq!(
        loaded_result.metadata_location.clone().unwrap(),
        original_metadata
    );

    // Cleanup - delete table from destination namespace
    tables
        .delete_table(&warehouse, dest_namespace.clone(), new_table_name.clone())
        .unwrap()
        .build()
        .send()
        .await
    ...
    let resp: Result<_, Error> = tables
        .load_table(&warehouse, dest_namespace.clone(), new_table_name)
        .unwrap()
        .build()
        .send()
        .await;
    ...
    tables
        .delete_namespace(&warehouse, source_namespace.clone())
        .unwrap()
        .build()
        .send()
        .await
    ...
    let resp: Result<_, Error> = tables
        .get_namespace(&warehouse, source_namespace)
        .unwrap()
        .build()
        .send()
        .await;
    ...
    tables
        .delete_namespace(&warehouse, dest_namespace.clone())
        .unwrap()
        .build()
        .send()
        .await
    ...
    let resp: Result<_, Error> = tables
        .get_namespace(&warehouse, dest_namespace)
        .unwrap()
        .build()
        .send()
        .await;
