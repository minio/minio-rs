...
/// Note: MinIO may not fully support nested/multi-level namespaces as AWS S3 Tables does.
...
async fn nested_namespace_create(ctx: TestContext) {
    ...
    tables
        .create_namespace(&warehouse, parent_ns.clone())
        .unwrap()
        .build()
        .send()
        .await
    ...
    let create_result = tables
        .create_namespace(&warehouse, nested_ns.clone())
        .unwrap()
        .build()
        .send()
        .await;

    let resp = match create_result {
        Ok(resp) => resp,
        Err(e) => {
            eprintln!(
                "> Server failed to create nested namespace: {:?}. Skipping test.",
                e
            );
    ...
    if created_ns != nested_ns_vec.as_slice() {
        // Server doesn't support nested namespaces
        eprintln!(
            "> Server returns flattened namespace (got {:?}, expected {:?}). Skipping nested namespace test.",
            created_ns, nested_ns_vec
        );
        // Clean up with the actual returned namespace
        let actual_ns = Namespace::try_from(created_ns.to_vec()).unwrap();
        tables
    ...
    let get_resp = tables
        .get_namespace(&warehouse, nested_ns.clone())
        .unwrap()
        .build()
        .send()
        .await
    ...
    tables
        .delete_namespace(&warehouse, nested_ns)
        .unwrap()
        .build()
        .send()
        .await
...
async fn rename_view_across_namespaces(ctx: TestContext) {
    ...
    create_warehouse_helper(&warehouse, &tables).await;
    create_namespace_helper(&warehouse, &source_ns, &tables).await;
    create_namespace_helper(&warehouse, &target_ns, &tables).await;
    ...
    let resp: ViewExistsResponse = tables
        .view_exists(&warehouse, target_ns.clone(), new_view_name.clone())
        .unwrap()
        .build()
        .send()
        .await
    ...
    let resp: ViewExistsResponse = tables
        .view_exists(&warehouse, source_ns.clone(), view)
        .unwrap()
        .build()
        .send()
        .await
    ...
    // Cleanup
    tables
        .drop_view(&warehouse, target_ns.clone(), new_view_name)
        .unwrap()
        .build()
        .send()
        .await
        .ok();
    delete_namespace_helper(&warehouse, &source_ns, &tables).await;
    delete_namespace_helper(&warehouse, &target_ns, &tables).await;
    delete_warehouse_helper(&warehouse, &tables).await;
}
...
async fn rename_table_destination_exists_fails(ctx: TestContext) {
    ...
    let result = tables
        .rename_table(&warehouse, &namespace, &table1, &namespace, &table2)
        .unwrap()
        .build()
        .send()
        .await;
...
async fn rename_view_destination_exists_fails(ctx: TestContext) {
    ...
    tables
        .create_view(&warehouse, &namespace, view2.clone(), schema, view_sql2)
        .unwrap()
        .build()
        .send()
        .await
    ...
    // Cleanup
    tables
        .drop_view(&warehouse, &namespace, view1)
        .unwrap()
        .build()
        .send()
        .await
        .ok();
    tables
        .drop_view(&warehouse, &namespace, view2)
        .unwrap()
        .build()
        .send()
        .await
...
async fn drop_nonexistent_table_handling(ctx: TestContext) {
    ...
    let result = tables
        .delete_table(&warehouse, &namespace, nonexistent_table)
        .unwrap()
        .build()
        .send()
        .await;
...
async fn drop_nonexistent_namespace_handling(ctx: TestContext) {
    ...
    let result = tables
        .delete_namespace(&warehouse, nonexistent_ns)
        .unwrap()
        .build()
        .send()
        .await;
