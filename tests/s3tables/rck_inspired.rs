...
use minio::s3tables::utils::{Namespace, TableName, ViewName};
...
/// Note: Nested namespaces are not yet supported by MinIO - test is ignored
#[ignore = "Nested namespaces not yet supported by MinIO"]
...
async fn nested_namespace_create(ctx: TestContext) {
    ...
    // MinIO may or may not support nested namespaces, both behaviors are valid
    if let Ok(resp) = result {
        // If supported, verify the namespace was created
        let created_ns = resp.namespace().unwrap();
        assert!(
            !created_ns.is_empty(),
            "Nested namespace should be created with non-empty name"
        );

        // Cleanup - delete child first
        tables
