...
async fn test_warehouse_trait_accessors(ctx: TestContext) {
    ...
    assert_eq!(resp.warehouse().unwrap(), warehouse.as_str());
    assert!(
...
async fn test_namespace_list_trait(ctx: TestContext) {
    ...
    // Create namespaces
    tables
        .create_namespace(&warehouse, &ns1)
        .unwrap()
        .build()
        .send()
        .await
    ...
    tables
        .create_namespace(&warehouse, &ns2)
        .unwrap()
        .build()
        .send()
        .await
    ...
    assert!(namespaces.iter().any(|ns| ns == &ns1));
    assert!(namespaces.iter().any(|ns| ns == &ns2));

    // Cleanup
    let _ = tables
        .delete_namespace(&warehouse, ns1)
        .unwrap()
        .build()
        .send()
        .await;
    let _ = tables
        .delete_namespace(&warehouse, ns2)
        .unwrap()
        .build()
        .send()
        .await;
