...
async fn test_warehouse_trait_accessors(ctx: TestContext) {
    ...
    assert_eq!(resp.warehouse_name().unwrap(), warehouse_name.as_str());
    assert!(
...
async fn test_namespace_trait_accessors(ctx: TestContext) {
    ...
    // Test that namespace_parts() returns the parsed response data
    let parsed_ns: Vec<String> = resp.namespace_parts().unwrap();
    assert_eq!(parsed_ns, namespace.as_slice());
...
async fn test_warehouse_list_trait(ctx: TestContext) {
    ...
    // Create warehouses
    tables
        .create_warehouse(warehouse1.clone())
        .build()
        .send()
        .await
    ...
    tables
        .create_warehouse(warehouse2.clone())
        .build()
        .send()
        .await
    ...
    let warehouse_names = resp.warehouses().expect("Failed to parse warehouses");
    assert!(
        warehouse_names
            .iter()
            .any(|w| w.as_str() == warehouse1.as_str())
    );
    assert!(
        warehouse_names
            .iter()
            .any(|w| w.as_str() == warehouse2.as_str())
    );
...
async fn test_namespace_list_trait(ctx: TestContext) {
    ...
    // Create namespaces
    tables
        .create_namespace(warehouse_name.clone(), ns1.clone())
        .build()
        .send()
        .await
    ...
    tables
        .create_namespace(warehouse_name.clone(), ns2.clone())
        .build()
        .send()
        .await
    ...
    assert!(namespaces.iter().any(|ns| ns == ns1.as_slice()));
    assert!(namespaces.iter().any(|ns| ns == ns2.as_slice()));

    // Cleanup
    let _ = tables
        .delete_namespace(warehouse_name.clone(), ns1)
        .build()
        .send()
        .await;
    let _ = tables
        .delete_namespace(warehouse_name.clone(), ns2)
        .build()
        .send()
        .await;
    let _ = tables.delete_warehouse(warehouse_name).build().send().await;
...
async fn test_table_list_trait(ctx: TestContext) {
    ...
    assert!(names.contains(&table1.as_str().to_string()));
    assert!(names.contains(&table2.as_str().to_string()));
...
async fn test_get_config_tables_fields_trait(ctx: TestContext) {
    ...
    let config_resp = tables
        .get_config(warehouse_name.clone())
        .build()
        .send()
        .await
