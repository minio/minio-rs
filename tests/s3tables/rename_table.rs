...
async fn rename_table_within_same_namespace(ctx: TestContext) {
    ...
    let original_metadata = result.metadata_location.clone().unwrap();
    ...
    let resp: LoadTableResponse = tables
        .load_table(&warehouse, &namespace, new_table_name.clone())
        .unwrap()
        .build()
        .send()
        .await
    ...
    // Cleanup
    tables
        .delete_table(&warehouse, &namespace, new_table_name.clone())
        .unwrap()
        .build()
        .send()
        .await
...
async fn rename_table_across_namespaces(ctx: TestContext) {
    ...
    create_warehouse_helper(&warehouse, &tables).await;
    create_namespace_helper(&warehouse, &source_namespace, &tables).await;
    create_namespace_helper(&warehouse, &target_namespace, &tables).await;

    // Create table in source namespace
    let schema = create_test_schema();
    let resp: CreateTableResponse = tables
        .create_table(&warehouse, source_namespace.clone(), &table, schema)
        .unwrap()
        .build()
        .send()
        .await
    ...
    let resp: Result<LoadTableResponse, Error> = tables
        .load_table(&warehouse, source_namespace.clone(), &table)
        .unwrap()
        .build()
        .send()
        .await;
    ...
    let resp: LoadTableResponse = tables
        .load_table(&warehouse, target_namespace.clone(), new_table_name.clone())
        .unwrap()
        .build()
        .send()
        .await
    ...
    // Cleanup
    tables
        .delete_table(&warehouse, target_namespace.clone(), new_table_name.clone())
        .unwrap()
        .build()
        .send()
        .await
        .unwrap();
    delete_namespace_helper(&warehouse, &source_namespace, &tables).await;
    delete_namespace_helper(&warehouse, &target_namespace, &tables).await;
    delete_warehouse_helper(&warehouse, &tables).await;
}
...
async fn rename_table_to_itself(ctx: TestContext) {
    ...
    let resp: RenameTableResponse = tables
        .rename_table(&warehouse, &namespace, &table, &namespace, &table)
        .unwrap()
        .build()
        .send()
        .await
...
async fn rename_table_to_nonexistent_namespace_fails(ctx: TestContext) {
    ...
    let resp: Result<RenameTableResponse, Error> = tables
        .rename_table(&warehouse, &namespace, &table, nonexistent_ns, &table)
        .unwrap()
        .build()
        .send()
        .await;
...
async fn rename_table_bidirectional(ctx: TestContext) {
    ...
    create_warehouse_helper(&warehouse, &tables).await;
    create_namespace_helper(&warehouse, &namespace1, &tables).await;
    create_namespace_helper(&warehouse, &namespace2, &tables).await;

    // Create table
    let schema = create_test_schema();
    let resp: CreateTableResponse = tables
        .create_table(&warehouse, namespace1.clone(), &table, schema)
        .unwrap()
        .build()
        .send()
        .await
        .unwrap();
    let original_metadata = resp.table_result().unwrap().metadata_location.unwrap();
    ...
    let resp: LoadTableResponse = tables
        .load_table(&warehouse, namespace1.clone(), &table)
        .unwrap()
        .build()
        .send()
        .await
    ...
    let resp: Result<LoadTableResponse, Error> = tables
        .load_table(&warehouse, namespace2.clone(), renamed_table.clone())
        .unwrap()
        .build()
        .send()
        .await;
    ...
    // Cleanup
    tables
        .delete_table(&warehouse, namespace1.clone(), &table)
        .unwrap()
        .build()
        .send()
        .await
        .unwrap();
    delete_namespace_helper(&warehouse, &namespace1, &tables).await;
    delete_namespace_helper(&warehouse, &namespace2, &tables).await;
    delete_warehouse_helper(&warehouse, &tables).await;
}
