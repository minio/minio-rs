...
fn is_unsupported_api(err: &Error) -> bool {
    match err {
        Error::S3Server(minio::s3::error::S3ServerError::HttpError(400, msg)) => {
            msg.contains("unsupported API call") || msg.contains("sort")
        }
        _ => false,
    }
}
...
/// Note: MinIO may not support sort order specification; test handles gracefully.
#[minio_macros::test(no_bucket)]
async fn create_table_with_sort_order(ctx: TestContext) {
    ...
    // Order ID 0 is reserved for unsorted, use 1+
    ...
    let resp: Result<CreateTableResponse, Error> = tables
        .create_table(&warehouse, &namespace, &table, schema)
        .unwrap()
    ...
    match resp {
        ...
        Err(ref e) if is_unsupported_api(e) => {
            eprintln!("> Sort order not supported by server, skipping test");
        }
...
async fn create_table_with_all_options(ctx: TestContext) {
    ...
    // Sort by timestamp descending (order ID 0 is reserved for unsorted)
    let sort_order: SortOrder = SortOrder {
        order_id: 1,
    ...
    match resp {
        ...
        Err(ref e) if is_unsupported_api(e) => {
            eprintln!("> Sort order/partition not supported by server, skipping test");
        }
        Err(e) => panic!("Unexpected error: {e:?}"),
    }
