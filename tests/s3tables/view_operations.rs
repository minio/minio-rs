...
async fn create_and_load_view(ctx: TestContext) {
    ...
    let sql = "SELECT id, name FROM test_table WHERE id > 0".to_string();
        ...
        Ok(create_resp) => {
            ...
            match create_resp.metadata() {
                Ok(metadata) => {
                    ...
                    let loaded_metadata = load_resp.metadata().unwrap();
                    assert_eq!(
                ...
                Err(e) if e.to_string().contains("invalid type: null") => {
                    eprintln!("Server returned null metadata for view, skipping verification");
                    // Try to drop view anyway in case it was created
                    let _ = tables
                        .drop_view(warehouse_name.clone(), namespace.clone(), view_name)
                        .build()
                        .send()
        ...
        Ok(create_resp) => {
            ...
            match create_resp.metadata() {
                Ok(initial_metadata) => {
                    ...
                    let updated_metadata = replace_resp.metadata().unwrap();
                    assert_eq!(
                ...
                Err(e) if e.to_string().contains("invalid type: null") => {
                    eprintln!("Server returned null metadata for view, skipping test");
                    // Try to drop view anyway in case it was created
                    let _ = tables
                        .drop_view(warehouse_name.clone(), namespace.clone(), view_name)
                        .build()
