...
async fn create_and_load_view(ctx: TestContext) {
    ...
    let create_resp: Result<CreateViewResponse, Error> = tables
        .create_view(&warehouse, &namespace, view.clone(), schema, sql)
        .unwrap()
        .dialect("spark")
        .build()
        .send()
...
async fn list_views_with_views(ctx: TestContext) {
    ...
    // Cleanup
    tables
        .drop_view(&warehouse, &namespace, view_name1)
        .unwrap()
        .build()
        .send()
        .await
        .unwrap();
    tables
        .drop_view(&warehouse, &namespace, view_name2)
        .unwrap()
        .build()
        .send()
        .await
...
async fn rename_view(ctx: TestContext) {
    ...
    let resp: ViewExistsResponse = tables
        .view_exists(&warehouse, &namespace, new_view_name.clone())
        .unwrap()
        .build()
        .send()
        .await
    ...
    // Cleanup
    tables
        .drop_view(&warehouse, &namespace, new_view_name)
        .unwrap()
        .build()
        .send()
        .await
