...
use minio::s3inventory::{
    DestinationSpec, Field, FilterSpec, GetInventoryConfigResponse, JobDefinition,
    LastModifiedFilter, ListInventoryConfigsJson, ListInventoryConfigsResponse, ModeSpec,
    NameFilter, OnOrOff, OutputFormat, Schedule, SizeFilter, VersionsSpec,
};
use minio_common::test_context::TestContext;

#[minio_macros::test(no_cleanup)]
async fn inventory_with_filters(ctx: TestContext, bucket_name: BucketName) {
    ...
    let resp: GetInventoryConfigResponse = ctx
        ...
        .await
        .unwrap();
    let config = resp.inventory_config().unwrap();

    assert_eq!(config.id, job_id);
    let yaml = config.yaml_def;
...
#[minio_macros::test(no_cleanup)]
async fn inventory_different_formats(ctx: TestContext, bucket_name: BucketName) {
    let dest_bucket_str = format!("{bucket_name}-dest");
    let bucket = bucket_name.clone();
    let dest_bucket = BucketName::new(&dest_bucket_str).unwrap();
    ...
    ctx.client
        .put_inventory_config(&bucket, "test-csv", csv_job)
        .unwrap()
        .build()
        .send()
        .await
    ...
    ctx.client
        .put_inventory_config(&bucket, "test-json", json_job)
        .unwrap()
        .build()
        .send()
        .await
    ...
    ctx.client
        .put_inventory_config(&bucket, "test-parquet", parquet_job)
        .unwrap()
        .build()
        .send()
        .await
    ...
    let job_ids = loop {
        ...
        let ids: Vec<String> = configs.items.iter().map(|item| item.id.clone()).collect();
