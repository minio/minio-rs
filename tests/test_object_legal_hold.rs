...
#[tokio::test(flavor = "multi_thread", worker_threads = 10)]
async fn object_legal_hold_s3express() {
    let ctx = TestContext::new_from_env();
    if !ctx.client.is_minio_express() {
    ...
    let resp: Result<MakeBucketResponse, Error> = ctx
        .client
        .make_bucket(&bucket_name)
        .object_lock(true)
        .send()
        .await;
    match resp {
        Err(Error::InvalidResponse(code, _)) => assert_eq!(code, 501),
        v => panic!("Expected error InvalidResponse(501): but got {:?}", v),
    }
    ...
    let resp: Result<DisableObjectLegalHoldResponse, Error> = ctx
        .client
        .disable_object_legal_hold(&bucket_name, &object_name)
        .send()
        .await;
    ...
    let resp: Result<IsObjectLegalHoldEnabledResponse, Error> = ctx
        .client
        .is_object_legal_hold_enabled(&bucket_name, &object_name)
        .send()
        .await;
