...
async fn setup_test_data(
    ...
    match client
        .create_bucket(BucketName::new(bucket)?)
        .build()
        .send()
        .await
    {
...
async fn run_benchmark(
    ...
    for i in 0..iterations {
        ...
        let result = client
            .get_object(BucketName::new(bucket)?, ObjectKey::new(&object_name)?)
            .build()
            .send()
            .await;
    ...
    for i in 0..iterations {
        ...
        let result = client
            .stat_object(BucketName::new(bucket)?, ObjectKey::new(&object_name)?)
            .build()
            .send()
            .await;
    ...
    for _ in 0..iterations {
        ...
        let mut stream = client
            .list_objects(BucketName::new(bucket)?)
            .build()
            .to_stream()
            .await;
    ...
    while let Some(object_result) = list_stream.next().await {
        if let Ok(obj_batch) = object_result {
            for obj in &obj_batch.contents {
                if client
                    .delete_object(BucketName::new(bucket)?, &obj.name)
                    .build()
                    .send()
                    .await
                    .is_ok()
