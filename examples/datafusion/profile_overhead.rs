...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let bucket = std::env::var("MINIO_BUCKET").unwrap_or_else(|_| "benchmark-test".to_string());
    let object = "test_data.parquet";
    ...
    for _ in 0..3 {
        let _ = minio_client
            ...
            .send()
            .await;
        let _ = object_store.get(&Path::from(object)).await;
    }
    ...
    profile_object_store(&object_store, object).await?;
    ...
    profile_range_requests(&minio_client, &object_store, &bucket, object).await?;

    profile_parallel_requests(&minio_client, &object_store, &bucket, object).await?;

    profile_head_requests(&minio_client, &object_store, &bucket, object).await?;
...
async fn profile_minio_rs(
    ...
    for _ in 0..ITERATIONS {
        // Measure build time
        let start = Instant::now();
        let builder = client.get_object(bucket, object).build();
...
async fn profile_fast_path(
    ...
    for _ in 0..ITERATIONS {
        ...
        let start = Instant::now();
        let response = client.get_object_fast(bucket, object, None).await?;
