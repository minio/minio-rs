...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match cli.command {
        ...
        Commands::Bench {
            ...
            let results: Vec<BenchmarkResult> = run_pushdown_benchmark(&config, iterations).await?;
        ...
        Commands::Compare {
            ...
            compare_pushdown_effectiveness(&config).await?;
        }
...
async fn run_pushdown_benchmark(
    ...
    let object_store: Arc<MinioObjectStore> = Arc::new(MinioObjectStore::new(
        minio_client,
