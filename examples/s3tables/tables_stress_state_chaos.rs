...
impl Config {
    fn from_env() -> Self {
        Self {
            ...
            region: env::var("TABLES_REGION").unwrap_or_else(|_| "us-east-1".to_string()),
    ...
    fn print(&self) {
        ...
        println!("    Polaris:    {}", base_paths::POLARIS);
        println!("    Nessie:     {}", base_paths::NESSIE);
    }
}
                    ...
                    if let (Ok(warehouse), Ok(namespace), Ok(table_name)) = (
                            ...
                            .await
                        {
                            Ok(resp) => {
                                ...
                                {
                                    state.add_plan_id(&wh, ns, &tbl, &plan_id);
                                }
                            }
                            Err(e) => {
                                metrics.record_error(operation.name(), e.to_string(), resource);
                ...
                if let Some((wh, ns, tbl, plan_id)) = state.random_plan_id() {
                    ...
                    if let (Ok(warehouse), Ok(namespace), Ok(table_name)) = (
                            ...
                            .await
                        {
                            Ok(_) => {
                                success = true;
                                state.remove_plan_id(&wh, &ns, &tbl, &plan_id);
                            }
                            Err(e) => {
                                metrics.record_error(operation.name(), e.to_string(), resource);
                            }
