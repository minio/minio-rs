...
impl TestContext {
    pub fn new_from_env() -> Self {
        ...
        if run_on_ci {
            ...
            const DEFAULT_IGNORE_CERT_CHECK: &str = "true";
    ...
    /// # Example
    /// ```no_run
    /// let (bucket, guard) = client.create_bucket_helper().await;
    /// println!("Created temporary bucket: {}", bucket);
    /// // The bucket will be removed when `guard` is dropped.
    /// ```
    pub async fn create_bucket_helper(&self) -> (BucketName, CleanupGuard) {
        ...
        let _resp = self
            ...
            .await
            .unwrap();
        let guard = CleanupGuard::new(self.client.clone(), bucket.clone());
        (bucket, guard)
    }
}
