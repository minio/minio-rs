...
pub fn create_post_policy_example(bucket: &str, object: &str) -> PostPolicy {
    let expiration: DateTime<Utc> = utc_now() + chrono::Duration::days(5);

    let mut policy = PostPolicy::new(BucketName::try_from(bucket).unwrap(), expiration).unwrap();
    policy.add_equals_condition("key", object).unwrap();
    policy
